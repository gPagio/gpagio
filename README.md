# 👨‍💻 Olá, eu sou o Guilherme Pagio! 👋
### Desenvolvedor Backend Java | Spring Boot | React

**Atuando desde 2022** no mercado de tecnologia, sou especializado no desenvolvimento de aplicações escaláveis dentro do ecossistema **Java**.

Meu foco é projetar e construir arquiteturas escaláveis e modernas, integrando backends em **Spring Boot** com interfaces em **React**.

Além do desenvolvimento de novas features, possuo vasta experiência em cenários críticos de otimização, tendo atuado com sucesso na modernização de sistemas e migração de rotinas complexas (PL/SQL para Java).

💡 **Impacto Real:** Em projetos recentes, implementei diversas melhorias no sistema, alcançando ganhos de performance de até **6.000%** em rotinas e processos críticos.

---

## 🛠️ Tech Stack

| Categoria | Tecnologias |
| :--- | :--- |
| **Backend** | ![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white) ![Spring Boot](https://img.shields.io/badge/-Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white) ![Swagger](https://img.shields.io/badge/-Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black) ![Postman](https://img.shields.io/badge/-Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white) |
| **Banco de Dados** | ![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white) ![Oracle](https://img.shields.io/badge/-Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white) ![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=for-the-badge&logo=mongodb&logoColor=white) |
| **DevOps & Cloud** | ![Docker](https://img.shields.io/badge/-Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white) ![Azure](https://img.shields.io/badge/-Azure-0078D4?style=for-the-badge&logo=microsoft-azure&logoColor=white) ![AWS](https://img.shields.io/badge/-AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white) |
| **Frontend** | ![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white) ![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB) ![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white) ![Figma](https://img.shields.io/badge/figma-%23F24E1E.svg?style=for-the-badge&logo=figma&logoColor=white) |

---

## 🚀 Experiência & Projetos

| Case / Projeto | Impacto & Tecnologia |
| :--- | :--- |
| **🐶 Toly Id (Lead Dev)** | Arquitetura completa de plataforma para identificação digital. Implementação de **CI/CD** com deploy em **Azure**. <br> *Techs: Java, Spring Boot, Docker, Swagger.* |
| **⚡ Otimização de Performance** | Refatoração estratégica de legado. **Resultado:** Redução de **6.000%** no load time e **600%** em rotinas de automação. <br> *Techs: Java, PL/SQL, Oracle.* |
| **🏭 Sistemas Industriais** | Desenvolvimento de soluções de controle de produção e integração via API com ERP Sankhya. <br> *Techs: Java, SQL, Integrações.* |

---

## ⚠️ Convenção de Nomenclatura dos Meus Repositórios
Para organização eu adotei uma convenção de nomenclatura para os repositórios do meu perfil, onde as regras são as seguintes:

| Prefixo | Descrição | Aplicação | Exemplo |
| --- | --- | --- | --- |
| bootcamp | Usado em projetos desenvolvidos durante eventos que participei | prefixo-promotor-nome-evento | bootcamp-rocketseat-todolist-springboot |
| curso | Usado em projetos desenvolvidos durante cursos que fiz | prefixo-instituicao-nome-curso | curso-alura-formacao-java |
| livro | Usado em projetos desenvolvidos durante estudos realizados em livros | prefixo-editora-nome-livro | livro-pearson-java-8-como-programar |
| multivix | Usado em projetos desenvolvidos na faculdade. | prefixo-nome-projeto | multivix-aulas-faculdade |
| projeto | Usado em projetos pessoais, sejam projetos reais ou com objetivo de aprendizado | prefixo-nome-projeto | projeto-experimentos-pessoais |
| trabalho | Usados em projetos desenvolvidos para o mercado de trabalho que estou inserido | prefixo-empresa-nome-projeto | trabalho-acme-modulo-erp |

> [!IMPORTANT]
> 1. Todos os repositórios seguem o padrão `kebab-case`.
> 1. Forks também são renomeados seguindo esta convenção.
> 1. Trabalhos da faculdade são organizados por semestre e bimestre conforme: `multivix-4s-2b-trabalho-(disciplina)`.

---

## 📫 Contato

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?style=for-the-badge&logo=Linkedin&logoColor=white)](https://www.linkedin.com/in/guilhermepagio/)
[![Gmail](https://img.shields.io/badge/-Gmail-c14438?style=for-the-badge&logo=Gmail&logoColor=white)](mailto:guilhermepagio.contato@gmail.com)
